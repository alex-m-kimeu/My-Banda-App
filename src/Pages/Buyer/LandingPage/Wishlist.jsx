...
body: JSON.stringify({ product_id: item.products.id }),
})
.then((response) => {
if (!response.ok) {
throw new Error("Network response was not ok");
...
<div key={`${item.products.id}-${index}`} className="rounded p-4 shadow-md">
<img
src={item.products.images && item.products.images.length > 0 ? item.products.images[0] : "/path/to/placeholder.jpg"}
alt={item.products.title}
className="w-full h-48 object-cover mb-4"
/>
<h3 className="text-lg font-semibold">{item.products.title}</h3>
<p className="text-lg text-Variant font-bold">${item.products.price}</p>
<button

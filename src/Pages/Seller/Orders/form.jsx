...
const OrderForm = () => {
      ...
      if (order && order.product_id) {
        try {
          ...
          const response = await fetch(
            `https://my-banda.onrender.com/products/${order.product_id}`,
            {
              headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://my-banda.onrender.com/orderByID/${order.id}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="p-4 sm:p-8 mx-auto mt-10 sm:mt-10 md:mt-10 lg:mt-30 xl:mt-30 lg:ml-0 sm:ml-[-20px] sm:w-[calc(100%-60px)]">
      <form className="sm:h-auto" onSubmit={handleSubmit}>
        ...
        <p>
          Product ID{" "}
          <span className="font-semibold"> #{productDetails.id}</span>
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mt-2 px-6 py-3 rounded shadow shadow-boxShadow">
          ...
          <div>
            ...
            <div className="overflow-hidden flex">
              <div className="flex flex-col mr-4 justify-center">
                <HiOutlineUser className="h-20 w-20  rounded bg-gray-100 p-1" />
              </div>
              <div className="p-2 sm:p-4">
                <p className="mb-2 text-gray-600">
                  <strong>Buyer ID:</strong> {buyerDetails.id}
                </p>
                <p className="mb-2 text-gray-600">
                  <strong>Name:</strong> {buyerDetails.username}
                </p>
                <p className="mb-2 text-gray-600">
                  <strong>Email:</strong> {buyerDetails.email}
                </p>
                <p className="mb-2 text-gray-600">
                  <strong>Phone:</strong> {buyerDetails.contact}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="overflow-hidden flex">
              <div className="flex flex-col mr-4 justify-center">
                <BsTruck className="h-20 w-20  rounded bg-gray-100 p-1" />
              </div>
              <div className="p-2 sm:p-4">
                <p className="mb-2 text-gray-600">
                  <strong>Company :</strong> {deliveryDetails.name}
                </p>
                <p className="mb-2 text-gray-600">
                  <strong>Company Location:</strong> {deliveryDetails.location}
                </p>
                <p className="mb-2 text-gray-600">
                ...
                <p className="mb-2 text-gray-600">
                  ...
                  <span className="bg-orange-100  ml-3 px-1 py-1 rounded">
          ...
          <div>
            ...
            <div className="flex mr-4">
              <div className="flex flex-col mr-4 justify-center h-44 ">
                <TbStatusChange className="h-20 w-20  rounded bg-gray-100 p-1" />
              </div>
              <div>
                {successMessage && (
                ...
                <div className="overflow-hidden cursor-pointer">
                  <div className="p-2 sm:p-4">
                    <select
                      ...
                      className="px-4 py-2 border rounded w-full sm:w-auto cursor-pointer"
                    >
        ...
        <div className="mt-6">
          <button
            type="submit"
            className="px-4 py-2 bg-Secondary text-white rounded hover:bg-yellow-600"
          >
            Update Order
          </button>
        </div>
